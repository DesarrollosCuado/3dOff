...
void GLWidget::initializeGL()
{
        ...
        vs = new GLVertexShader(QDir::currentPath()+"/../phong.vert");
        fs = new GLFragmentShader(QDir::currentPath()+"/../phong.frag");
        glsl=new GLProgram;
        glsl->attach(*vs);
        if(glsl->failed()){
...
void GLWidget::changeList()
{
        ...
        glBegin(GL_LINES);
            for(int i=0;i<normals.size();i++)
            {
                    if(clockwise)
                        ...
                        glNormal3f(normalsCCW[i].x,normalsCCW[i].y,normalsCCW[i].z);
                    punto3d r=(vertex[triangles[i][0]]+vertex[triangles[i][1]]+vertex[triangles[i][2]])*0.333333333f;
                    punto3d in=normals[i]*0.02f+r;
    ...
    glNewList(primitiveList+2,GL_COMPILE); // Lines
        glBegin(GL_LINES);
            for(int i=0;i<triangles.size();i++)
            {
                    ...
                    glVertex3f(vertex[triangles[i][0]].x, vertex[triangles[i][0]].y, vertex[triangles[i][0]].z);
                    glVertex3f(vertex[triangles[i][1]].x, vertex[triangles[i][1]].y, vertex[triangles[i][1]].z);
                    glVertex3f(vertex[triangles[i][2]].x, vertex[triangles[i][2]].y, vertex[triangles[i][2]].z);
                    glVertex3f(vertex[triangles[i][0]].x, vertex[triangles[i][0]].y, vertex[triangles[i][0]].z);
                    glVertex3f(vertex[triangles[i][1]].x, vertex[triangles[i][1]].y, vertex[triangles[i][1]].z);
                    glVertex3f(vertex[triangles[i][2]].x, vertex[triangles[i][2]].y, vertex[triangles[i][2]].z);
            }
    ...
    glNewList(primitiveList+3,GL_COMPILE); // Trianges/Solid Planar
        glBegin(GL_TRIANGLES);
            for(int i=0;i<triangles.size();i++)
            {
                    ...
                    if(clockwise) glNormal3f(normals2[triangles[i][0]].x, normals2[triangles[i][0]].y, normals2[triangles[i][0]].z);
                    else glNormal3f(normals2CCW[triangles[i][0]].x, normals2CCW[triangles[i][0]].y, normals2CCW[triangles[i][0]].z);

                    glTexCoord2f(vertex[triangles[i][0]].x+0.5f, vertex[triangles[i][0]].y+0.5f);
                    glVertex3f(vertex[triangles[i][0]].x, vertex[triangles[i][0]].y, vertex[triangles[i][0]].z);

                   if(clockwise) glNormal3f(normals2[triangles[i][1]].x, normals2[triangles[i][1]].y, normals2[triangles[i][1]].z);
                    else glNormal3f(normals2CCW[triangles[i][1]].x, normals2CCW[triangles[i][1]].y, normals2CCW[triangles[i][1]].z);

                    glTexCoord2f(vertex[triangles[i][1]].x+0.5f, vertex[triangles[i][1]].y+0.5f);
                    glVertex3f(vertex[triangles[i][1]].x, vertex[triangles[i][1]].y, vertex[triangles[i][1]].z);


                    if(clockwise) glNormal3f(normals2[triangles[i][2]].x, normals2[triangles[i][2]].y, normals2[triangles[i][2]].z);
                    else glNormal3f(normals2CCW[triangles[i][2]].x, normals2CCW[triangles[i][2]].y, normals2CCW[triangles[i][2]].z);

                    glTexCoord2f(vertex[triangles[i][2]].x+0.5f, vertex[triangles[i][2]].y+0.5f);
                    glVertex3f(vertex[triangles[i][2]].x, vertex[triangles[i][2]].y, vertex[triangles[i][2]].z);
            }
        glEnd();
    glEndList();
    ...
    glNewList(primitiveList+4,GL_COMPILE); // Bounding Box
        ...
        glBegin(GL_LINES);
            //Cara 1
            glVertex3f(me.x,ma.y,me.z);
            glVertex3f(ma.x,ma.y,me.z);
            glVertex3f(me.x,me.y,me.z);
            glVertex3f(ma.x,me.y,me.z);
            glVertex3f(ma.x,ma.y,me.z);
            glVertex3f(ma.x,me.y,me.z);
            glVertex3f(me.x,ma.y,me.z);
            glVertex3f(me.x,me.y,me.z);
            //Cara 2
            glVertex3f(me.x,ma.y,ma.z);
            glVertex3f(ma.x,ma.y,ma.z);
            glVertex3f(me.x,me.y,ma.z);
            glVertex3f(ma.x,me.y,ma.z);
            glVertex3f(ma.x,ma.y,ma.z);
            glVertex3f(ma.x,me.y,ma.z);
            glVertex3f(me.x,ma.y,ma.z);
            glVertex3f(me.x,me.y,ma.z);
    ...
    glNewList(primitiveList+5,GL_COMPILE); // Trianges/Solid Cilindrico
        glBegin(GL_TRIANGLES);
            float S,T,x,y;
            for(int i=0;i<triangles.size();i++)
            {
                    ...
                    T=vertex[triangles[i][0]].z+0.5f;
                    x=vertex[triangles[i][0]].x+0.5f;
                    y=vertex[triangles[i][0]].y+0.5f;
                    S=atan2(y,x)/(2*PI);
                   ...
                   if(clockwise) glNormal3f(normals2[triangles[i][1]].x, normals2[triangles[i][1]].y, normals2[triangles[i][1]].z);
                    ...
                    T=vertex[triangles[i][1]].z+0.5f;
                    x=vertex[triangles[i][1]].x+0.5f;
                    y=vertex[triangles[i][1]].y+0.5f;
                    S=atan2(y,x)/(2*PI);
                    ...
                    T=vertex[triangles[i][2]].z+0.5f;
                    x=vertex[triangles[i][2]].x+0.5f;
                    y=vertex[triangles[i][2]].y+0.5f;
                    S=atan2(y,x)/(2*PI);
                    ...
                    //glTexCoord2f(vertex[triangles[i][2]].x+0.5f, vertex[triangles[i][2]].z+0.5f);
...
void GLWidget::paintGL()
{
    ...
    if(luz[0].active){
        if(modelLight==2) glsl->setInt(QString("LuzAct[0]"),1);
        glEnable(GL_LIGHT0);
    ...
    if(luz[1].active){
        ...
        if(modelLight==2) glsl->setInt(QString("LuzAct[1]"),0);
        glDisable(GL_LIGHT1);
    }

    if(luz[2].active){
        if(modelLight==2) glsl->setInt(QString("LuzAct[2]"),1);
        if(pointL){
            glBegin(GL_POINTS);
                    glNormal3f(0.0f, 0.0f, 0.0f);
        ...
        if(modelLight==2) glsl->setInt(QString("LuzAct[2]"),0);
        glDisable(GL_LIGHT2);
    }

    if(luz[3].active) {
        if(modelLight==2) glsl->setInt(QString("LuzAct[3]"),1);
        if(pointL){
            glBegin(GL_POINTS);
                    glNormal3f(0.0f, 0.0f, 0.0f);
        ...
        if(modelLight==2) glsl->setInt(QString("LuzAct[3]"),0);
        glDisable(GL_LIGHT3);
    }

    if(luz[4].active) {
        if(modelLight==2) glsl->setInt(QString("LuzAct[4]"),1);
        if(pointL){
            glBegin(GL_POINTS);
                    glNormal3f(0.0f, 0.0f, 0.0f);
        ...
        if(modelLight==2) glsl->setInt(QString("LuzAct[4]"),0);
        glDisable(GL_LIGHT4);
    }

	if(fog){
        if(modelLight==2) glsl->setInt(QString("fogA"),1);
		glEnable(GL_DEPTH_TEST);
		GLfloat fogColor[4];
		...
		glEnable (GL_FOG); //enable the fog
                ...
                if(modelLight==2) glsl->setInt(QString("fogA"),0);
		glDisable (GL_FOG); 
	}
